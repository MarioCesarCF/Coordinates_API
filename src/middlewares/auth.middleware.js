...
class AuthMiddleware {
    ...
    try {
      ...
      if (schema !== "Bearer") {
        return res
          .status(401)
          .send({
            message: "A palavra Bearer não foi encontrada no cabeçalho.",
          });
      }
  ...
  authenticationClient = (req, res, next) => {
    try {
      const { authorization } = req.headers;
      ...
      jwt.verify(token, process.env.KEY_TOKEN, async (error, decoded) => {
        ...
        const client = await clientRepository.findByIdRepository(decoded.id);
